...
@Directive({
  selector: '[mtEmojiPickerCaretEmitter]',
  host: {
...
export class EmojiPickerCaretDirective {
  @Output('mtEmojiPickerCaretEmitter') caretEmitter = new EventEmitter<CaretEvent>();
  ...
  updateCaretDueMutation() {
    const cEvent = CaretEvent.generateCaretEvent(this.win, this.doc, this._el.nativeElement);
    const textMovement = cEvent.textContent.length - this._lastCaretEvent.textContent.length;
